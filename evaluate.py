...
async def run(task: str):
    async with httpx.AsyncClient(timeout=30) as client:
        ...
        response = await client.post("http://localhost:8003/run", params={"task": task})
        try:
...
async def read(path: str):
    async with httpx.AsyncClient(timeout=30) as client:
        response = await client.get(f"http://localhost:8003/read?path={path}")
...
async def a10(email, **kwargs):
    ...
    return True
async def b12(email, **kwargs):
    """
    Evaluate Task B12 by checking a valid and an invalid file path.
    This evaluation assumes that your system will call B12_check_path (from tasksB.py)
    to check if a path is secure and write "True" or "False" to an output file.
    """
    # Instruct the system to check a valid file path and write the result.
    await run("Check if the file path `/data/test-valid.txt` is inside /data using B12, and write the result (True or False) to `/data/b12-valid.txt`")
    valid_result = await read("/data/b12-valid.txt")
    if valid_result.strip().lower() != "true":
        return mismatch("/data/b12-valid.txt", "True", valid_result)

    # Instruct the system to check an invalid file path and write the result.
    await run("Check if the file path `/etc/passwd` is inside /data using B12, and write the result (True or False) to `/data/b12-invalid.txt`")
    invalid_result = await read("/data/b12-invalid.txt")
    if invalid_result.strip().lower() != "false":
        return mismatch("/data/b12-invalid.txt", "False", invalid_result)

    return True

async def b12(email, **kwargs):
    """
    Tests whether B12 correctly distinguishes valid vs. invalid paths.
    B12 returns True if filepath starts with /data, otherwise False.
    """

    # 1. Check a valid path
    await run(
        "Check if the file path `/data/test-valid.txt` is inside /data using B12, "
        "and write the result (True or False) to `/data/b12-valid.txt`"
    )
    valid_result = await read("/data/b12-valid.txt")
    ...
    await run(
        "Check if the file path `/etc/passwd` is inside /data using B12, "
        "and write the result (True or False) to `/data/b12-invalid.txt`"
    )
    invalid_result = await read("/data/b12-invalid.txt")
...
async def main(email: str):
    score, total = 0, 0
    for task in [a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, b12, b3, b6]:
        total += 1
        try:
